...
import defenses.defensegan.tflib as tflib
from defenses.defensegan.tflib.mnist import *
from defenses.defensegan.datasets.utils import get_generators
from defenses.defensegan.models.base_model import AbstractModel
from defenses.defensegan.models.dataset_models import mnist_generator, mnist_discriminator
from defenses.defensegan.utils.misc import ensure_dir
...
class DefenseGANBase(AbstractModel):
    ...
    def reconstruct(
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
            ...
            # I set seed so that the reconstruction restarts are always the same.

            z_hat = tf.get_variable(
                'z_hat_rec_{}'.format(reconstructor_id),
                shape=[batch_size * self.rec_rr, self.latent_dim],
                dtype=tf.float32,
                initializer=tf.random_normal_initializer(stddev=np.sqrt(1.0 / self.latent_dim)),
                collections=[tf.GraphKeys.LOCAL_VARIABLES]
            )
            ...
            #    'z_hat_rec_{}'.format(reconstructor_id),
            #    dtype=tf.float32,
            #    initializer=tf.random_normal(seed=2,
            #        stddev=np.sqrt(1.0 / self.latent_dim), shape=(64*self.rec_rr, self.latent_dim))[:batch_size*self.rec_rr],
            #    collections=[tf.GraphKeys.LOCAL_VARIABLES]
            #)
