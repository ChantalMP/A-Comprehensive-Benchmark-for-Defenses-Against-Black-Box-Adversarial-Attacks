...
def get_Mahalanobis_score_adv(model, test_data, test_label, num_classes, net_type, sample_mean, precision, layer_index, magnitude, dataset = "cifar10"):
    '''
    ...
    for data_index in range(int(np.ceil(test_data.size(0)/batch_size))):
...
def get_LID(model, test_clean_data, test_adv_data, test_noisy_data, test_label, num_output):
    ...
    for data_index in range(int(np.floor(test_clean_data.size(0)/batch_size))):
        ...
        for i in range(num_output):
            out_features[i] = out_features[i].view(out_features[i].size(0), out_features[i].size(1), -1)
            out_features[i] = torch.mean(out_features[i].data, 2)
            X_act.append(np.asarray(out_features[i].cpu(), dtype=np.float32).reshape((out_features[i].cpu().size(0), -1)))
        ...
        for i in range(num_output):
            out_features[i] = out_features[i].view(out_features[i].size(0), out_features[i].size(1), -1)
            out_features[i] = torch.mean(out_features[i].data, 2)
            X_act_adv.append(np.asarray(out_features[i].cpu(), dtype=np.float32).reshape((out_features[i].cpu().size(0), -1)))
        ...
        for i in range(num_output):
            out_features[i] = out_features[i].view(out_features[i].size(0), out_features[i].size(1), -1)
            out_features[i] = torch.mean(out_features[i].data, 2)
            X_act_noisy.append(np.asarray(out_features[i].cpu(), dtype=np.float32).reshape((out_features[i].cpu().size(0), -1)))
