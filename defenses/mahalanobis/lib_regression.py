...
def my_stuff(regressor, X, Y):
    ...
    ones_pred = regressor.predict_proba(x_ones)[:, 1]
    zero_pred = regressor.predict_proba(x_zeros)[:, 1]

    print("clean: ")
    print(np.average(ones_pred))
    print("adv: ")
    print(np.average(zero_pred))
    print("end")
